...
export async function setAvailabilitySlots(formData) {
  ...
  try {
    ...
    // give all the slots of the doctor 
    ...
    // If slots exist, mark out the empty slots which are free not booked by patients by   (slot) => !slot.appointment
    ...
    if (existingSlots.length > 0) {
      ...
      // delete the empty slots becoz doctor has finsihed its chamvber and slots need to be deleted so that no patient can book it further
    ...
    // Create new availability slot means morning shift is over , create for evening
